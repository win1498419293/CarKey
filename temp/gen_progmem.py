...
<div class="grid">
  <div class="act-card" onclick="doApi('/api/unlock')"><div class="act-title">🔓</div><div class="act-sub">Unlock</div></div>
  <div class="act-card" onclick="doApi('/api/lock')"><div class="act-title">🔒</div><div class="act-sub">Lock</div></div>
  <div class="act-card" onclick="doApi('/api/horn')"><div class="act-title">📢</div><div class="act-sub">Horn</div></div>
  <div class="act-card" onclick="doApi('/api/window')"><div class="act-title">🪟</div><div class="act-sub">Window</div></div>
</div>
<div class="pair-card" id="pairCard">
  <div class="pair-header">
    <div class="pair-title">📱 蓝牙配对</div>
    <div class="pair-status pair-none" id="pBadge">未配对</div>
  </div>
  <div class="pair-info" id="pInfo">点击开始配对，在手机蓝牙中搜索设备</div>
  <div style="margin-top:8px">
    <button class="pair-btn-sm btn-primary" id="pStart" onclick="startPair()">开始配对</button>
    <button class="pair-btn-sm btn-danger" id="pStop" onclick="stopPair()" style="display:none">停止</button>
    <button class="pair-btn-sm btn-ghost" onclick="clearPair()">清除</button>
  </div>
</div>
...
<div class="overlay" id="authOverlay">
  <div class="modal-box">
    <div class="modal-title">🔑 启动密码</div>
    <input type="password" id="authPwd" placeholder="6位密码" maxlength="6" autofocus>
    <div class="btn-row">
      <button class="btn btn-ghost" onclick="closeAuth()">取消</button>
      <button class="btn btn-primary" onclick="doStart()">确认</button>
    </div>
...
<div class="overlay" id="settingsOverlay">
  <div class="modal-box">
    ...
    <label>WiFi 密码</label><input type="password" id="cfgPass">
    <label>蓝牙名称</label><input type="text" id="cfgBt">
    <label>启动密码</label><input type="password" id="cfgPwd" maxlength="6">
    <div class="toggle-row"><span>BLE 扫描</span><input type="checkbox" id="cfgBleScan"></div>
    <div class="toggle-row"><span>安全认证</span><input type="checkbox" id="cfgSec"></div>
    <div class="btn-row">
...
function doStart(){
  ...
  if(!p||p.length!==6){E('authMsg').innerText='请输入6位密码';return}
  ...
  fetch('/api/start_engine?pwd='+p).then(function(r){
    ...
    return r.text().then(function(t){if(r.status===401)E('authMsg').innerText='密码错误';else E('authMsg').innerText=t})
  }).catch(function(){addLog('Network error','err')});
}
function doApi(url){
  addLog('Action: '+url,'info');
  fetch(url).then(function(r){return r.text()}).then(function(t){addLog('OK: '+t,'ok')}).catch(function(){addLog('Fail','err')});
}
...
function updatePairStatus(){
  fetch('/api/ble/pairing/status').then(function(r){return r.json()}).then(function(d){
    if(d.pairing){
      E('pBadge').className='pair-status pair-ing';E('pBadge').innerText='配对中...';
      E('pInfo').innerText='请在手机蓝牙中搜索设备';
      E('pStart').style.display='none';E('pStop').style.display='inline-block';
    }else if(d.paired){
      E('pBadge').className='pair-status pair-ok';E('pBadge').innerText='已配对';
      E('pInfo').innerHTML='设备: '+d.name+' ('+d.mac+')';
      E('pStart').style.display='inline-block';E('pStop').style.display='none';
    }else{
      E('pBadge').className='pair-status pair-none';E('pBadge').innerText='未配对';
      E('pInfo').innerText='点击开始配对，在手机蓝牙中搜索设备';
      E('pStart').style.display='inline-block';E('pStop').style.display='none';
    }
  }).catch(function(){});
}
